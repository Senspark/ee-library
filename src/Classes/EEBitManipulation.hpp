...
template<class T>
constexpr T clearLowestBit(T n) noexcept {
    return n & (n - 1);
}

template<class T>
constexpr T isolateLowestBit(T n) noexcept {
    // Maybe also use: return n & -n.
    return n ^ clearLowestBit(n);
}

template<>
constexpr std::uint64_t isolateHighestBit<std::uint64_t>(std::uint64_t n) noexcept {
    n |= n >> 1;
    n |= n >> 2;
    n |= n >> 4;
    n |= n >> 8;
...
template<class T>
constexpr std::uint32_t bitScanForwardSlow(T n) noexcept {
...
template<>
constexpr std::uint32_t bitScanForward<std::uint64_t>(std::uint64_t n) noexcept {
...
template<>
constexpr std::uint32_t bitScanReverse<std::uint64_t>(std::uint64_t n) noexcept {
    n |= n >> 1;
    n |= n >> 2;
    n |= n >> 4;
    n |= n >> 8;
...
template<>
constexpr std::uint32_t countTrailingZero<std::uint64_t>(std::uint64_t n) noexcept {
    return n ? bitScanForward(n) : 64;
}

template<>
constexpr std::uint32_t countLeadingZero<std::uint64_t>(std::uint64_t n) noexcept {
    return n ? (bitScanReverse(n) ^ 63) : 64;
}

template<class T>
constexpr std::uint32_t bitPopCount(T n) noexcept {
...
template<class T>
constexpr std::uint32_t bitPopCountKernighan(T n) noexcept {
