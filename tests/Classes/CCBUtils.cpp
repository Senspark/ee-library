...
void CCBUtils::initLoaders() {
    ...
    library->registerNodeLoader(
        "BlurTestScene",
        ee::GenericLoader<TestScene, cocosbuilder::NodeLoader>::loader());
}

cocos2d::Scene* CCBUtils::openScene(const std::string& filename) {
    auto reader = new cocosbuilder::CCBReader(
        cocosbuilder::NodeLoaderLibrary::getInstance());
    auto scene = reader->createSceneWithNodeGraphFromFile(filename.c_str());
    reader->release();
    auto transition =
        cocos2d::TransitionFade::create(0.5f, scene, cocos2d::Color3B(0, 0, 0));
